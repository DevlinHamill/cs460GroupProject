...
public class MembersList extends AppCompatActivity {
    ...
    private void getUsers(){
        ...
        database.collection(Constants.KEY_COLLECTION_MEMBERS).get()
                .addOnCompleteListener(task -> {
                    loading(false);
                    String currentUserID = preferenceManager.getString(Constants.KEY_MEMBER_ID);
                    if(task.isSuccessful() && task.getResult() != null){
                        ...
                        for(QueryDocumentSnapshot queryDocumentSnapshot: task.getResult()){
                            ...
                            member.name = queryDocumentSnapshot.getString(Constants.KEY_NAME);
                            member.profileImage = queryDocumentSnapshot.getString(Constants.KEY_IMAGE);
                            member.id = queryDocumentSnapshot.getString(Constants.KEY_MEMBER_ID);
